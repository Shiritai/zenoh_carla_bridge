...
use carla::geom::{Location, Rotation, Transform, Vector3D};
use carla::{
...
impl<'a> VehicleBridge<'a> {
    ...
    fn apply_pending_initialpose(&mut self) {
        ...
        if let Some(iso) = pending.as_ref() {
            // carla 0.13's Transform::from_na wants its own nalgebra version, so
            // decompose into scalars here instead of crossing the crate boundary.
            let (roll, pitch, yaw) = iso.rotation.euler_angles();
            self.actor.set_transform(&Transform {
                location: Location::new(iso.translation.x, iso.translation.y, iso.translation.z),
                rotation: Rotation::new(pitch.to_degrees(), yaw.to_degrees(), roll.to_degrees()),
            });
            // Clear velocities so the simulator doesn't fight the teleport
            // with leftover momentum.
            self.actor
                .set_target_velocity(&Vector3D::new(0.0, 0.0, 0.0));
            self.actor
                .set_target_angular_velocity(&Vector3D::new(0.0, 0.0, 0.0));
            log::info!(
                "[{}] teleported to ({:.2}, {:.2}, {:.2}) via /initialpose",
                self.vehicle_name,
                iso.translation.x,
                iso.translation.y,
                iso.translation.z,
...
/// Convert a ROS PoseWithCovarianceStamped to a Carla-frame Isometry3.
///
/// ROS REP-103: right-handed, x forward, y **left**, z up.
/// Carla / UE4:  left-handed,  x forward, y **right**, z up.
///
/// Reflecting the y axis flips the sign of rotations around the x and z axes
